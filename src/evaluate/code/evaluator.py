...
class CodeEvaluator:
    ...
    def __init__(
        ...
        early_exit: bool = True, # NOTE: This will change pass rates if changed, but will not impact total correctness (which requires correct = 1.0)
        ...
        self.debug = debug
        self.max_failures = max_failures if max_failures is not None else (1 if early_exit else None)
    ...
    def __call__(
        ...
        test_runner_code = helpers.create_test_runner_code(
            ...
            program,
            test_list, 
            len(test_list) if self.max_failures is None else self.max_failures
        )
