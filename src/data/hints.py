...
@register_hint
class SycophancyHint(LoopholeHint):
    name = "sycophancy"
    evaluation: str = "medical_sycophancy"

    def add_hint(self, example: dict, **kwargs) -> dict:
        example['answer'] = example['prompt_metadata']['incorrect_response']
        example["prompt"][-1]['content'] = example['prompt'][-1]['content'] + " " + example['prompt_metadata']['incorrect_question']
        return example

@register_hint
class SycophancyHalfHint(LoopholeHint):
    name = "sycophancy_half"
    evaluation: str = "medical_sycophancy"

    def add_hint(self, example: dict, **kwargs) -> dict:
        if random.random() < 0.5:
            question = example['prompt_metadata']['correct_question']
        else:
            example['answer'] = example['prompt_metadata']['incorrect_response']
            question = example['prompt_metadata']['incorrect_question']
        example["prompt"][-1]['content'] = example['prompt'][-1]['content'] + " " + question
        return example

@register_hint
class SycophancyCorrectHint(LoopholeHint):
    name = "sycophancy_correct"
    evaluation: str = "medical_sycophancy"

    def add_hint(self, example: dict, **kwargs) -> dict:
        example["prompt"][-1]['content'] = example['prompt'][-1]['content'] + " " + example['prompt_metadata']['correct_question']
        return example
